...
@patch("apps.worker.src.pipeline.ResumeRepository")
@patch("hiron.ai_usage.repository.AIUsageRepository")
async def test_parse_resume_idempotent_retry(
    mock_ai_repo_cls,
    mock_resume_repo_cls,
):
    """Verify that an idempotent retry returns early and does not generate duplicate telemetry."""
    session = AsyncMock()
    tenant_id = uuid.uuid4()
    resume_id = uuid.uuid4()
    ...
    mock_ai_repo = mock_ai_repo_cls.return_value
    mock_ai_repo.create_usage_log.assert_not_called()
...
async def test_parse_resume_trigger_failure_swallowed(
    ...
    mock_qstash_publisher.publish = AsyncMock(side_effect=Exception("Network error"))
