...
@router.post("/qstash/resumes/parse", dependencies=[Depends(verify_qstash_signature)])
async def parse_resume_webhook(
    ...
    set_tenant_context(str(payload.tenant_id))
    ...
    try:
        ...
        await session.commit()
        logger.warning("Deterministic parse failure", error=str(exc))
...
@router.post("/qstash/embeddings/candidate", dependencies=[Depends(verify_qstash_signature)])
async def generate_candidate_embedding_webhook(
    payload: CandidateEmbeddingPayload,
    session: AsyncSession = Depends(get_db_session)
) -> dict[str, str]:
...
@router.post("/qstash/embeddings/job", dependencies=[Depends(verify_qstash_signature)])
async def generate_job_embedding_webhook(
    payload: JobEmbeddingPayload,
    session: AsyncSession = Depends(get_db_session)
) -> dict[str, str]:
