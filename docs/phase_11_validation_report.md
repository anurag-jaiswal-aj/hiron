...
2. **MissingGreenlet Fix:** Updated `apps/api/hiron/notes/service.py` (`update_note`) to run an explicit, asynchronous `await self.note_repo.get_note_by_id()` *after* `session.commit()` but before returning the serialized response, safely pre-loading all required attributes. `archive_note` was left alone as it correctly returned an HTTP 204 without serialization.
