...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        # 1. AUTHENTICATE
        print("\n1. AUTHENTICATING...")
        token_a_admin, user_a_admin = await login(client, data["admin_a_email"], password, tenant_a_id)
        token_a_rec, user_a_rec = await login(client, data["rec_a_email"], password, tenant_a_id)
        token_b_admin, user_b_admin = await login(client, data["admin_b_email"], password, tenant_b_id)
        ...
        # Create Candidate
        cand_data = {
            "full_name": f"Val Cand {run_id}",
            "email": f"val-cand-{run_id}@example.com"
        }
        resp = await client.post(f"{API_BASE}/candidates", json=cand_data, headers=headers(token_a_admin))
        resp.raise_for_status()
        ...
        if stages:
            ...
            resp = await client.post(f"{API_BASE}/candidates/{cand_id}/jobs", json={"job_id": job_id, "stage_id": stage_id}, headers=headers(token_a_admin))
            if resp.status_code == 200:
                print("   ✅ Linked Candidate to Job")

        # Create Note (by Recruiter!)
        note_data = {
            "content": f"Test note by recruiter {run_id}"
        }
        resp = await client.post(f"{API_BASE}/candidates/{cand_id}/notes", json=note_data, headers=headers(token_a_rec))
        if resp.status_code == 200:
        ...
        assert len(logs_a_admin) > 0, "Admin A sees no logs!"
        ...
        assert all((log.get("actor") or {}).get("id") == data["rec_a_id"] for log in logs_a_rec), "Recruiter sees other actions!"
        ...
        assert len(logs_b) == 0, "Admin B sees Tenant A logs!"
        print("   ✅ Admin B isolated from Tenant A logs")

        # 4. FILTERS & PAGINATION
        print("\n4. TESTING FILTERS...")
        # valid filter zero results
        resp = await client.get(f"{API_BASE}/audit-logs?entityType=score", headers=headers(token_a_admin))
        assert len(resp.json()["data"]) == 0
        print("   ✅ Valid filter with zero results verified")

        # invalid UUID format
        resp = await client.get(f"{API_BASE}/audit-logs?actorId=not-a-uuid", headers=headers(token_a_admin))
        assert resp.status_code in [400, 422], "Invalid UUID did not return 4xx"
        print("   ✅ Invalid UUID rejected correctly")

        # 5. BEFORE / AFTER VERIFICATION
        print("\n5. VERIFYING BEFORE / AFTER PAYLOADS...")
        job_update_log = next((l for l in logs_a_admin if l["action"] == "job_updated"), None)
        assert job_update_log is not None, "Could not find job update log"
        changes = job_update_log.get("changes", {})
        assert "before" in changes and "after" in changes, "Missing before/after in changes"
        assert changes["before"].get("department") == "Engineering"
        assert changes["after"].get("department") == "Product"
        ...
        r2 = await client.put(f"{API_BASE}/audit-logs/{log_id}", json={"action": "hacked"}, headers=headers(token_a_admin))
        assert r1.status_code in [403, 404, 405], f"DELETE allowed! Status: {r1.status_code}"
        assert r2.status_code in [403, 404, 405], f"PUT allowed! Status: {r2.status_code}"
